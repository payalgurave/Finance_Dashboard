...
const NAV = [
  ...
  {
    to: '/profile', label: 'Profile', roles: ['viewer', 'analyst', 'admin'],
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden">
      <aside className="w-64 bg-slate-900 flex flex-col shrink-0 shadow-2xl">
        ...
        <div className="p-4 space-y-3">
          ...
          <button
            onClick={toggle}
            className="w-full flex items-center justify-between px-3 py-2.5 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm text-slate-400 hover:text-white transition-all"
          >
            <span>{dark ? '☀️ Light Mode' : '🌙 Dark Mode'}</span>
            <div className={`w-9 h-5 rounded-full transition-colors ${dark ? 'bg-indigo-500' : 'bg-slate-600'} relative`}>
          ...
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-slate-400 hover:text-rose-400 bg-slate-800/50 hover:bg-rose-500/20 rounded-xl text-sm transition-all duration-200"
          >
